...
export default function TutorialPage() {
  ...
  let isBodyIndsideRect, isLHandInsideTarget, isRHandInsideTarget, isLKneeInsideTarget, isRKneeInsideTarget
  ...
  function onPoseResults(results) {
    ...
    if (results.poseLandmarks) {
      ...
      if (lEyeLm && rEyeLm && lHandLm && rHandLm
        && lKneeLm && rKneeLm && lFootLm && rFootLm){
        ...
        isLHandInsideTarget = isLmInsideTargets(lHandLmX, lHandLmY, lHandTargetX, lHandTargetY, targetWidth, targetHeight);
        isRHandInsideTarget = isLmInsideTargets(rHandLmX, rHandLmY, rHandTargetX, rHandTargetY, targetWidth, targetHeight);
        isLKneeInsideTarget = isLmInsideTargets(lKneeLmX, lKneeLmY, lKneeTargetX, lKneeTargetY, targetWidth, targetHeight);
        isRKneeInsideTarget = isLmInsideTargets(rKneeLmX, rKneeLmY, rKneeTargetX, rKneeTargetY, targetWidth, targetHeight);
  ...
  function isLmInsideTargets(lmX, lmY, targetX, targetY, targetWidth, targetHeight){
      if (lmX >= targetX && lmX <= targetX + targetWidth &&
        lmY >= targetY && lmY <= targetY + targetHeight) {
        return true;
      }
      else{
        return false;
  ...
  return (
    ...
    >
      ...
      <Box position={"relative"} width={"500px"} height={"700px"} style={{
          transform: "scaleX(-1)"}}>
        <WebCam webcamRef={webcamRef} canvasRef={canvasRef} />
        <Frame  borderColor={"green"} w={rectWidth} h={rectHeight} x={rectX} y={rectY}/>
        <Target x={lHandTargetX} y={lHandTargetY} alt={"red"} src={"/assets/images/red.png"} />
        <Target x={rHandTargetX} y={rHandTargetY} alt={"red"} src={"/assets/images/red.png"} />
        <Target x={lKneeTargetX} y={lKneeTargetY} alt={"blue"} src={"/assets/images/blue.png"} />
        <Target x={rKneeTargetX} y={rKneeTargetY} alt={"blue"} src={"/assets/images/blue.png"} />
      </Box>
    </Box>
  );
}
